...
class BRGame:
    ...
    def run_event(self, all_losers: list, event: Event=None, players: list=None) -> dict:
        ...
        return {'name': event.name, 'value': _template}
    
    def _build_message(self, fields: list) -> str:
        """Builds a message to post in chat given a list of fields generated by events"""
        content = ""
    ...
    async def _generate_event(self) -> None:
        ...
        if len(self.participants) >= 64:
            ...
            embed = [{'name': 'Players', 'value': f'{len(self.participants)} players remaining.', 'inline': False}]
            await self._parent._bot.send_embed_message(self._setup_message.channel, f"Round {self._round}", fields=embed)
            content = self._build_message(fields)
            await self._parent._bot.send_message(self._setup_message.channel, content)
