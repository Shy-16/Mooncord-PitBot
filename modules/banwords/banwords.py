...
class Banwords:
    ...
    async def do_timeout(self, banword: str, context: CommandContext, match = None) -> None:
        ...
        # Issue the timeout
        self._pitbot.add_timeout(user=user, guild_id=context.guild.id,
        ...
        # Add the roles
        await user.add_roles(*context.ban_roles, reason='Automatic timeout issued for typing a banned word: ({})'.format(banword['word']))
