...
class BotConfig(Command):
    ...
    async def send_banwords(self, context: CommandContext) -> None:
        fields = [
            {'name': 'Banwords', 'value': "Please use the website to configure banwords.", 'inline': False}
        ]
    ...
    async def _show_server_configuration(self, context: CommandContext) -> None:
        ...
        if admin_text == '':
            admin_text = 'No admin roles specified.'
        ...
        mod_text = '\r\n'.join(mod_role_text)
        if mod_text == '':
            mod_text = 'No mod roles specified.'
        ...
        ban_text = '\r\n'.join(ban_role_text)
        if ban_text == '':
            ban_text = 'No ban roles specified.'
        ...
        sub_text = '\r\n'.join(sub_role_text)
        if sub_text == '':
            sub_text = 'No sub roles specified.'
        ...
        fields = [
            ...
            {'name': 'banwords', 'value': "Use the website to configure banwords.", 'inline': False},
    ...
    async def _set_log_channel(self, context: CommandContext) -> None:
        if context.channel_mentions:
            channel = context.channel_mentions[0]

            guild_config = self._bot.db.update_server_configuration(context.guild, {'log_channel': channel.id})
            await self._bot.update_guild_configuration(context.guild)

            fields= [
                {'name': 'log_channel', 'value': f"Log Channel was properly set up to: <#{channel.id}>", 'inline': False}
            ]
    ...
    async def _set_override_silent(self, context: CommandContext) -> None:
        value = context.params[2].lower in ["true", "yes", "1"]

        self._bot.db.update_server_configuration(context.guild, {'override_silent': value})
        await self._bot.update_guild_configuration(context.guild)
    ...
    async def _add_role(self, context: CommandContext) -> None:
        ...
        if which not in ['admin_roles', 'mod_roles', 'ban_roles', 'sub_roles']:
            fields= [
                {'name': 'add *_roles', 'value': "Available lists are: `admin_roles`, `mod_roles`, `ban_roles`, `sub_roles`", 'inline': False}
            ]
        ...
        self._bot.db.add_role_to_guild(context.guild, which, str(role.id))
        await self._bot.update_guild_configuration(context.guild)

        fields= [
            {'name': f'add {which}', 'value': f"<@&{role.id}> ({role.id}) was properly added to server config.", 'inline': False}
        ]
    ...
    async def _remove_role(self, context: CommandContext) -> None:
        ...
        self._bot.db.remove_role_from_guild(context.guild, which, str(role.id))
        await self._bot.update_guild_configuration(context.guild)

        fields= [
            {'name': f'add {which}', 'value': f"<@&{role.id}> ({role.id}) was properly removed from server config.", 'inline': False}
        ]
