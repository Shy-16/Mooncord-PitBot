...
class Timeout(Command):
	...
	def __init__(self, bot, permission='mod', dm_keywords=list(), skip_strike=False):
		"""
		@bot: Sayo
	...
	async def execute(self, context):
		...
		timeout_info = self._bot.db.get_user_timeout(mention)
		extended = False
		...
		if not timeout_info:
			...
			timeout_info = self._bot.db.update_user_timeout(mention, {'time': new_time})
			extended = True
	...
	async def send_help(self, context):
		...
		await self._bot.send_embed_message(context.channel, "Timeout User", fields=fields)
